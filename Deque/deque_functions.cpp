...
void inputStudentData(Student &s) {
    try {
        ...
        while (true) {
            ...
            if (grade >= 1 && grade <= 10) {
                s.grades.push_back(grade);  // `deque` veikia taip pat kaip `vector`
            } else {
        ...
        while (true) {
            ...
            if (cin.fail()) {
                cin.clear();
                cin.ignore(100, '\n');
                throw std::runtime_error("Netinkama ivestis! Prasome ivesti skaiciu (1-10).");
            }
...
double calculateMedian(const Student &s) {
    ...
    deque<int> sortedGrades = s.grades;
    sort(sortedGrades.begin(), sortedGrades.end());  // Rikiavimas vietoje
...
void printStudents(const deque<Student> &students, bool useMedian) {
    ...
    // Kadangi deque neturi tiesioginio sort() metodo, sukuriu laikina vector rikiavimui
    vector<Student> sortedStudents(students.begin(), students.end());
    ...
    for (const auto &s : sortedStudents) {
        cout << left << setw(15) << s.name << setw(15)  << s.surname << "  " 
             << fixed << setprecision(2) << calculateFinalGrade(s, useMedian) << endl;
    }
...
void readFromFile(deque<Student> &students, const string &filename) {
    try {
        ...
        string line;
        getline(file, line);  // Praleidziame antraste
        ...
        while (getline(file, line)) {  
            ...
            int grade;
            deque<int> tempGrades;  // Naudojame deque vietoj vector
            while (iss >> grade) {
...
void saveResultsToFile(deque<Student> students, const string& filename, bool showAverage, bool showMedian){
    try {
        ...
        // Rikiavimas pagal pasirinkimą naudojant `deque`
        if (sortOption == '1') {
...
void generateStudentFiles() {
    ...
    cout << "Bendras failų kūrimo laikas: " << fixed << setprecision(5) << totalTime << " s\n";
...
void testDataProcessing(const string& filename, int strategy) {
    ...
    // 1. Duomenų nuskaitymas is failo
    auto start_time = high_resolution_clock::now();
